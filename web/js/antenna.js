...
var antennaGeometry = function(holder, w, h, scale, coords, ini_x_off_, ini_y_off_, confs_, confs_title, bg) {
    ...
    , coordsToSegments = function(coords) {
        var i = 0, segments = [], color, pts, j, scale, s, e, dif, len;
    ...
    , drawAxis = function() {
        ...
        geometry_set.push(r_line(raphael, x1, y1, x2, y2).attr({ stroke: redb, stroke_width: 1 }));
        x2 = x1 + (x2 - x1) * 1.25;
        y2 = y1 + (y2 - y1) * 1.25;
        geometry_set.push(raphael.text(x2, y2, "X").attr({ stroke: redb, stroke_width: 1 }));
        e = proj_model_view_mtr.x(Vector.j).x(30).add(_off);
        x2 = e.e(1);
        y2 = e.e(2);
        geometry_set.push(r_line(raphael, x1, y1, x2, y2).attr({ stroke: greenb, stroke_width: 1 }));
        x2 = x1 + (x2 - x1) * 1.25;
        y2 = y1 + (y2 - y1) * 1.25;
        geometry_set.push(raphael.text(x2, y2, "Y").attr({ stroke: greenb, stroke_width: 1 }));
        e = proj_model_view_mtr.x(Vector.k).x(30).add(_off);
        x2 = e.e(1);
        y2 = e.e(2);
        geometry_set.push(r_line(raphael, x1, y1, x2, y2).attr({ stroke: blueb, stroke_width: 1 }));
        x2 = x1 + (x2 - x1) * 1.25;
        y2 = y1 + (y2 - y1) * 1.25;
        geometry_set.push(raphael.text(x2, y2, "Z").attr({ stroke: blueb, stroke_width: 1 }));
    }
    ...
    , drawConfigurations = function() {
        ...
        charsize = min((w - 20.0) / (l + confs.length + 1), 8), start = (w - charsize * l) / 2 - 30;
	...
	, fit = function() {
	    var 
            ...
            , org = (bounding_box[1].add(bounding_box[0])).x(.5);
    ...
    , drawButtons = function() {
        ...
        var gray1 = "#888", gray2 = "#ccc", black = "#000", none = "none"
            ...
            , front_full = { fill: bgcolor, "stroke-width": 2, "fill-opacity": .75 }
            , rear_full = { fill: bgcolor, "stroke-width": .5, "stroke-dasharray": "-" }
            , front_empty = { fill: "none", "stroke-width": 2, "fill-opacity": .5 }
            , c, p, btn_set, inch_btn, mm_btn
            ;
        ...
        btn_set = raphael.set();
        btn_set.push(
		raphael.path("M0,3l15,-3l9,5l-15,3z").attr(front_full)
            );
        btn_set.push(
            raphael.path("M0,3l9,5v16l-9,-5z").attr(front_full)
            );
        btn_set.push(
            raphael.path("M9,8v16l15,-3v-16z").attr(front_full)
            );
        applyHover(btn_set, undef, gray1, bgcolor).click(function(event) { model_view_mtr = Matrix.RotationX(radians(15)).x(Matrix.RotationY(radians(-60)).x($M([[0, 1, 0], [0, 0, 1], [1, 0, 0]]))); that.redraw(); })
        ...
        btn_set.push(
            applyHover(raphael.path("M0,8v16h16v-16z").attr(front_full), undef, gray1, bgcolor)
            .click(function(event) { model_view_mtr = $M([[0, 1, 0], [0, 0, 1], [1, 0, 0]]); that.redraw(); })
            );
        btn_set.push(
            applyHover(raphael.path("M0,8h16l8,-8h-16z").attr(front_full), undef, gray1, bgcolor)
            .click(function(event) { model_view_mtr = $M([[0, 1, 0], [-1, 0, 0], [0, 0, 1]]); that.redraw(); })
            );
        btn_set.push(
            applyHover(raphael.path("M16,8v16l8,-8v-16z").attr(front_full), undef, gray1, bgcolor)
            .click(function(event) { model_view_mtr = $M([[-1, 0, 0], [0, 0, 1], [0, 1, 0]]); that.redraw(); })
            );
        btn_set.translate(w - 178, h - 28).attr({ stroke: "#000", "stroke-linejoin": "round" });

        btn_set = raphael.set()
        btn_set.push(
            applyHover(raphael.path("M0,8v16l8,-8v-16z").attr(rear_full), undef, gray2, bgcolor)
            .click(function(event) { model_view_mtr = $M([[1, 0, 0], [0, 0, 1], [0, -1, 0]]); that.redraw(); })
            );
        btn_set.push(
            applyHover(raphael.path("M0,24h16l8,-8h-16z").attr(rear_full), undef, gray2, bgcolor)
            .click(function(event) { model_view_mtr = $M([[0, 1, 0], [1, 0, 0], [0, 0, -1]]); that.redraw(); })
            );
        btn_set.push(
            applyHover(raphael.path("M8,0h16v16h-16z").attr(rear_full), undef, gray2, bgcolor)
            .click(function(event) { model_view_mtr = $M([[0, -1, 0], [0, 0, 1], [-1, 0, 0]]); that.redraw(); })
            );
    ...
    this.redraw = function() {
        ...
        for (prop in fg_lines)
            fg_lines[prop].attr({ stroke: prop, "stroke-width": line_width });
        for (prop in fg_points)
            fg_points[prop].attr({ stroke: prop, fill: prop });
...
, AntennaHPattern = function(holder, size, channels, models, sym, model_names, colors) {
    size = Math.max(100, size - 20);
    this.x = size / 2;
    this.y = this.x + (models.length > 1 ? 20 : 0);
	...
	drawChartBG = function() {
	    ...
	    if (models.length > 1) {
	        for (i = 0; i != models.length; ++i) {
	            ...
	            raphael.text((i + .5) * size / models.length, 18, sym ? ("F: " + p[0] + "dBi, B: " + p[p.length - 1] + "dBi") : ("F: " + p[0] + "dBi, B: " + p[Math.round(p.length / 2 - .5)] + "dBi")).attr(fontSize(9));
	            raphael.text((i + .5) * size / models.length, 28, p.minValue() + " < dBi < " + p.maxValue()).attr(fontSize(9));
	        }
